...
get_eql_plot2 <- function(
  ...
  p <-
    ...
    pivot_longer(cols = c(all_of(strategies %>% gsub("eql_", "", .)))) %>%
    mutate(name = factor(name)) %>%
    mutate(name = fct_relevel(name, strategies %>% gsub("eql_", "", .))) %>%
    ggplot(aes(x = Date, y = value, col = name)) +
    geom_line() +
    theme_bw() +
    labs(x = labX, y = "") +
    theme(legend.title = element_blank(), legend.position = c(legX, legY))
