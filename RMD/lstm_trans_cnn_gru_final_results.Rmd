...
do_test3 <- function(dt, use_logret = T, use_DM = T) {
  ...
  if (use_DM) {
    ...
    # Test for differences in Sharpe
    #ctr <- list(type = 2, ttype = 1, pBoot = 2, hac = F)
    #result  <- PeerPerformance::sharpeTesting(e1, e2, control = ctr)
    #result2 <- c(result$tstat, result$pval)
    
    # standard significance test for H0: alpha = 0  vs. H1: alpha <> 0
    # in regression
    m1 <- lm(e1~e2)
    result <- summary(m1)$coefficients[1, c(3, 4)]
    result2 <- c(result[1], result[2])
...
e3 <- as.data.frame(eq_r[[2]])[, "eql_cnn"]
e4 <- as.data.frame(eq_r[[2]])[, "eql_gru"]
e5 <- as.data.frame(eq_r[[2]])[, "Close"]
if (0) {
  r1 <- diff(log(e1))
  r2 <- diff(log(e2))
  r3 <- diff(log(e3))
...
library(SharpeR)
sr_test(
  r1, r4, zeta = 2, ope = 365,
  alternative = "two.sided")[c("statistic", "p.value")]


library(MCS)
loss <- -cbind(r1, r2, r3, r4, r5) 
mcs <- MCSprocedure(loss, alpha = 0.05, B = 5000)
summary(mcs)
